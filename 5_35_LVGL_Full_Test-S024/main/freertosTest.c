...
void freertosTestFun(void){
	...
	while (1) {
		...
		printf("freertosTestFun running--------\n");
    }
}
...
void timerTest(void){
    ...
    if (!queue) {
        ESP_LOGE(TIEMRTAG, "Creating queue failed");
        return;
    }

    ESP_LOGI(TIEMRTAG, "Create timer handle");
    ...
    ESP_LOGI(TIEMRTAG, "Enable timer");
    ESP_ERROR_CHECK(gptimer_enable(gptimer));

    ESP_LOGI(TIEMRTAG, "Start timer, stop it at alarm event");
    ...
    if (xQueueReceive(queue, &ele, pdMS_TO_TICKS(2000))) {
        ESP_LOGI(TIEMRTAG, "Timer stopped, count=%llu", ele.event_count);
    } else {
        ESP_LOGW(TIEMRTAG, "Missed one count event");
    }

    ESP_LOGI(TIEMRTAG, "Set count value");
    ESP_ERROR_CHECK(gptimer_set_raw_count(gptimer, 100));
    ESP_LOGI(TIEMRTAG, "Get count value");
    uint64_t count;
    ESP_ERROR_CHECK(gptimer_get_raw_count(gptimer, &count));
    ESP_LOGI(TIEMRTAG, "Timer count value=%llu", count);

    // before updating the alarm callback, we should make sure the timer is not in the enable state
    ESP_LOGI(TIEMRTAG, "Disable timer");
    ...
    ESP_LOGI(TIEMRTAG, "Start timer, auto-reload at alarm event");
    ...
    while (record) {
        if (xQueueReceive(queue, &ele, pdMS_TO_TICKS(2000))) {
            ESP_LOGI(TIEMRTAG, "Timer reloaded, count=%llu", ele.event_count);
            record--;
        } else {
            ESP_LOGW(TIEMRTAG, "Missed one count event");
        }
    }
    ESP_LOGI(TIEMRTAG, "Stop timer");
    ...
    ESP_LOGI(TIEMRTAG, "Start timer, update alarm value dynamically");
    ...
    while (record) {
        if (xQueueReceive(queue, &ele, pdMS_TO_TICKS(2000))) {
            ESP_LOGI(TIEMRTAG, "Timer alarmed, count=%llu", ele.event_count);
            record--;
        } else {
            ESP_LOGW(TIEMRTAG, "Missed one count event");
        }
    ...
    ESP_LOGI(TIEMRTAG, "Disable timer");
    ESP_ERROR_CHECK(gptimer_disable(gptimer));
    ESP_LOGI(TIEMRTAG, "Delete timer");
